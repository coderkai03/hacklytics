...
def extract_audio_features(video_path: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {
            "quality_metrics": {
                "volume_level": rms_mean,
                "volume_consistency": float(1 - rms_std / rms_mean if rms_mean > 0 else 0),
                "high_frequency_content": zcr_mean,
                "frequency_variation": float(zcr_std / zcr_mean if zcr_mean > 0 else 0)
            },
            "quality_scores": {
                "volume_quality": "good" if rms_mean > 0.1 else "variable",
                "dynamic_range": "wide" if dynamic_range > 1.5 else "narrow",
                "frequency_quality": "rich" if zcr_mean > 0.1 else "basic",
                "overall_quality": "high" if (rms_mean > 0.1 and zcr_mean > 0.1) else "medium"
            },
            "dynamic_analysis": {
